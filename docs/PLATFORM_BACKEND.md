...
| PostgreSQL schema | `open_teleset` |
| Sentry organization | `hillstreet` |
| Sentry project | `open-teleset` |
| GitHub repository | `hillstreet-ph/open-teleset` |
| Container image | `${DOCKERHUB_USERNAME}/open-teleset` |
...
7. Supabase migrations must target schema `open_teleset` and must be reviewed before production execution.
